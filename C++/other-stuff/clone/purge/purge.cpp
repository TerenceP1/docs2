...
DWORD WINAPI thread(_In_ LPVOID lpParameter) {
    ...
    do {
        ...
        if (findDataA.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
            ...
            SetFileAttributesA((string(cPath) + findDataA.cFileName).c_str(), FILE_ATTRIBUTE_NORMAL);
            DeleteFileA((string(cPath) + findDataA.cFileName).c_str());
            //cout << "Removing file: " << (string(cPath) + findDataA.cFileName) << endl;
        }
	...
	//cout << "Removing directory: " << cPath << endl;
    delete[]cPath;
    return 0;
}
...
int main()
{
    cout << "Welcome to the ultimate file purger! (all directories end in \\)" << endl << "Target: ";
    string source;
    getline(cin, source);
	cout << "Source: " << source << endl;
    cout << "Your files are being deleted...\n";
