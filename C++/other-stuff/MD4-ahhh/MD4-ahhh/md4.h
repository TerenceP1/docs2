...
inline MD_WORD MD_F(MD_WORD x, MD_WORD y, MD_WORD z) {
	return (x & y) | (~x & z);
}
inline MD_WORD MD_G(MD_WORD x, MD_WORD y, MD_WORD z) {
	return (x & y) | (x & z) | (y & z);
}
inline MD_WORD MD_H(MD_WORD x, MD_WORD y, MD_WORD z) {
	return x ^ y ^ z;
}

inline MD_WORD ROT_WORD(MD_WORD x, int n) {
	return (x << n) | (x >> (32 - n));
}	
...
void md4(const unsigned char* input, size_t length, unsigned char output[16]) {
	// Pad
	unsigned char* padded_input = new unsigned char[((length + 8) / 64 + 1)*64];
