...
model.train()
engine.say(f"Please input your parameters.")
engine.runAndWait()
while True:
    trainNm=int(input("How many times to train? "))
    bSz=int(input("Batch size? "))
    engine.say(f"Beginning the training with {trainNm} epoches and a batch size of {bSz}. Beginning the training with {trainNm} epoches and a batch size of {bSz}. Beginning the training with {trainNm} epoches and a batch size of {bSz}. Beginning the training with {trainNm} epoches and a batch size of {bSz}.")
    engine.runAndWait()
    print("Number of times trained: 0",end="",flush=True)
    ls=0
    for i in range(trainNm):
        ...
        loss.backward()
        optimizer.step()
        print("\x1b["+str(len(str(i)))+"D"+str(i+1),end="",flush=True)
    print("")
    print(f"Loss: {ls}")
    torch.save(model.state_dict(),"model.pth")
    torch.save(optimizer.state_dict(),"optimizer.pth")
    engine.say("Hello! Your training finished. Awaiting further intructions. Hello! Your training finished. Awaiting further intructions. Hello! Your training finished. Awaiting further intructions. Hello! Your training finished. Awaiting further intructions.")
    engine.runAndWait()